...
#define alignment_up(a, size) ((a+size-1)/size)
...
}__attribute__((aligned (BMPMANAGER_ALIGNSIZE))) bmp_store_t;
...
static int bmp_manager_reset(){
	...
	startblks=16384-alignment_up(sizeof(bmp_store_t),BMPMANAGER_ALIGNSIZE);
	readblks=alignment_up(sizeof(bmp_store_t),BMPMANAGER_ALIGNSIZE);
...
int bmp_manager_init(const char* part){
	...
	if(store->magic!=BMP_MNGR_MAGIC){ 
		...
		//check start already store one splash bmp
			...
			//ret = flash_read(bmppart,0,head,BMPMANAGER_ALIGNSIZE);  //use mmc commond
		sprintf(cmdbuffer,"mmc read 0x%x 0x%x 0x%x",head,8192,1);
    	printf("%s\n",cmdbuffer);
    	run_command(cmdbuffer,0);
...
unsigned long bmp_manager_readbmp(const char* name,void* data,unsigned long size){
	...
	sprintf(cmdbuffer,"mmc read 0x%x 0x%x 0x%x",data,8192+bmp.start,(size>bmp.size)?bmp.size:size);
    printf("%s\n",cmdbuffer);
	...
	if(!bmp_manager_getbmp(name,&bmp)){
		...
		if(store->count>(location+1)){
			...
			//test space is enough
				//space = entry->length-used-movesize-alignment_up(sizeof(bmp_store_t),BMPMANAGER_ALIGNSIZE);//--allenyao
			space=8192-used-alignment_up(sizeof(bmp_store_t),BMPMANAGER_ALIGNSIZE);
			...
			sprintf(cmdbuffer,"mmc read 0x%x 0x%x 0x%x",(void*)BMPMANAGER_LOAD_ADDR,movestart+8192,movesize);
    		printf("%s\n",cmdbuffer);
    		run_command(cmdbuffer,0);
			...
			sprintf(cmdbuffer,"mmc write 0x%x 0x%x 0x%x",(void*)BMPMANAGER_LOAD_ADDR,mystart+8192,movesize);
    		printf("%s\n",cmdbuffer);
    		run_command(cmdbuffer,0);

			sprintf(cmdbuffer,"mmc write 0x%x 0x%x 0x%x",data,movesize+mystart+8192,size);
    		printf("%s\n",cmdbuffer);
    		run_command(cmdbuffer,0);
			...
			//flash_write(entry,entry->length-alignment_up(sizeof(bmp_store_t),BMPMANAGER_ALIGNSIZE),
				//store,alignment_up(sizeof(bmp_store_t),BMPMANAGER_ALIGNSIZE));
			sprintf(cmdbuffer,"mmc write 0x%x 0x%x 0x%x",store,startblks,readblks);
    		printf("%s\n",cmdbuffer);
    		run_command(cmdbuffer,0);
			...
			sprintf(cmdbuffer,"mmc write 0x%x 0x%x 0x%x",data,8192+used,size);
    		printf("%s\n",cmdbuffer);
    		run_command(cmdbuffer,0);
			//update new size
